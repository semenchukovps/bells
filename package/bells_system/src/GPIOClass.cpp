...
void GPIOClass::scan_gpio_input(){
  getval_gpio(input_state);
	if(input_state == "0"){
		usleep(20000);
		getval_gpio(input_state);
    input_state == "0" ? state = BUTTON_PRESSED :	state = BUTTON_NOT_PRESSED;
  }
}

void GPIOClass::scan_gpio_state(){
	
}

GPIOStates GPIOClass::get_gpio_state(){
	return state;	
